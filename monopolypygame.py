...
def move_token(pointer,dice,player_pos,curr_player):
    ...
    if pointer[curr_player] <= 9:
        ...
        if curr_loan[curr_player]!=0:
            money[curr_player]-=curr_loan[curr_player]+(2/100)*curr_loan[curr_player]
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not dice_rolled and 370 <= mouse_x <= 440 and 370 <= mouse_y <= 410  :
                    ...
                    if money_values[pointer[curr_player]]!=-1:
                        ...
                        #     pointer_to_jail=tycoons.index("Jail")
                            
                        #     move_token(pointer[pointer_to_jail],dice,player_pos,curr_player)
                            # print(f"pointer: {pointer}, type: {type(pointer)}")
                            # print(f"curr_player: {curr_player}, type: {type(curr_player)}")
